...
describe('Test Library', function () {
    ...
    it ("Should calculate value percentages correctly", async function() {
        ...
        expect(result).to.equal(1 * (10 ** PERCENTAGE_DECIMALS));
        result = await library.connect(user).valueIsWhatPercentOf(ethers.utils.parseUnits("100", "ether"), userInvestmentAmount);
        expect(result).to.equal(100 * (10 ** PERCENTAGE_DECIMALS));
        result = await library.connect(user).valueIsWhatPercentOf(ethers.utils.parseUnits("45", "ether"), userInvestmentAmount);
        expect(result).to.equal(45 * (10 ** PERCENTAGE_DECIMALS));
    })

    it ("Should subtract percentages correctly", async function() {
        //var result = await library.connect(user).subtractPercentage(userInvestmentAmount, 1 * (10 ** PERCENTAGE_DECIMALS)); // 1%
        //expect(result).to.equal(ethers.utils.parseUnits("99", "ether"));
        var result = await library.connect(user).subtractPercentage(userInvestmentAmount, 45 * (10 ** PERCENTAGE_DECIMALS));
    ...
    it ("Should add percentages correctly", async function() {
        var result = await library.connect(user).addPercentage(userInvestmentAmount, 1 * (10 ** PERCENTAGE_DECIMALS));
        expect(result).to.equal(ethers.utils.parseUnits("101", "ether"));
        result = await library.connect(user).addPercentage(userInvestmentAmount, 450 * (10 ** PERCENTAGE_DECIMALS));
        expect(result).to.equal(ethers.utils.parseUnits("550", "ether"));
        result = await library.connect(user).addPercentage(ethers.utils.parseUnits("45", "ether"), 45 * (10 ** PERCENTAGE_DECIMALS));
        expect(result).to.equal(ethers.utils.parseUnits("65.25", "ether"));
    })

    it ("Should take percentages correctly", async function() {
        var result = await library.connect(user).percentageOf(userInvestmentAmount, 1 * (10 ** PERCENTAGE_DECIMALS));
        expect(result).to.equal(ethers.utils.parseUnits("1", "ether"));
        result = await library.connect(user).percentageOf(userInvestmentAmount, 450 * (10 ** PERCENTAGE_DECIMALS));
        expect(result).to.equal(ethers.utils.parseUnits("450", "ether"));
        result = await library.connect(user).percentageOf(ethers.utils.parseUnits("45", "ether"), 45 * (10 ** PERCENTAGE_DECIMALS));
        expect(result).to.equal(ethers.utils.parseUnits("20.25", "ether"));
        // Test half a percent: 0.5%
        result = await library.connect(user).percentageOf(ethers.utils.parseUnits("45", "ether"),
                                                          (5 * (10 ** PERCENTAGE_DECIMALS)) / 10);
    ...
    it ("Should calculate the kelly bet correctly", async function() {
        ...
        var result = await library.connect(user).kellyFraction(60 * (10 ** PERCENTAGE_DECIMALS), lossPercent, gainPercent);
        expect(result).to.be.equal(BigNumber.from("100000000")); // 100%

        lossPercent = 10 * (10 ** 6);
        gainPercent = 100 * (10 ** 6);
        var result1 = await library.connect(user).kellyFraction(40 * (10 ** PERCENTAGE_DECIMALS), lossPercent, gainPercent);
